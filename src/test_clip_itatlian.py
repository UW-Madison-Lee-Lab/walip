...
from ops import get_clip_image_features, get_clip_text_features
...
def evaluate(data_name, language='ita'):
    ...
    if data_name == 'cifar100':
        ...
        dataloader = DataLoader(image_dataset, batch_size=512, shuffle=False, drop_last=False, num_workers=4)
    else:
        data_dir = '/mnt/nfs/work1/mccallum/dthai/tuan_data/imagenet/val'
        dataset = ImageFolder(data_dir, val_preprocess)
        dataloader = DataLoader(dataset, batch_size=32, shuffle=False, drop_last=False, num_workers=4)
    ...
    for batch_idx, (inputs, labels) in enumerate(dataloader):
        ...
        for x in inds:
            ...
            if t not in indices:
                indices[t] = []
            indices[t].append(x + batch_idx * 128)
