...
def get_recall(dico, scores):
    ...
    correct1, correct10, total = 0, 0, 0
    ...
    print("Recall@1 {:.3f} Recall@10: {:.3f}".format(correct1/total*100, correct10/total*100))

def align_words(matching_method, dico, scores, c=0.5):
                ...
                for m in range(k):
                    if s[0][i, m] > threshold:
                        ...
                        if dico is not None and s[1][i, m] in d[dico[i, 0].item()]:
                            correct += 1
            if dico is not None:
                print("Precision@1 ", correct, total, correct/total*100)
            return lst
...
def robust_procrustes(X, Y, c=0.1):
    ...
    for j in range(2):
        ...
        # t = torch.quantile(alphas, c)
        # inds = np.where(alphas.cpu().numpy() > t.cpu().numpy())[0]
        # alphas = alphas/torch.norm(alphas) 
        # alphas = alphas.softmax(dim=0)
        for i in range(n):
            # a = min(10, alphas[i].item())
            D[i, i] = alphas[i]**0.5
        X1 = D @ X1
        Y1 = D @ Y1
        M = Y1.T @ X1
